...
class BlobGC {
 public:
  BlobGC(std::vector<BlobFileMeta*>&& blob_files,
         NublobCFOptions&& _titan_cf_options, bool need_trigger_next, uint64_t scan_speed=0);
  ...
  const NublobCFOptions& titan_cf_options() { return titan_cf_options_; }
 ...
 private:
  ...
  NublobCFOptions titan_cf_options_;
