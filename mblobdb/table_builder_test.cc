...
class FileManager : public BlobFileManager {
 public:
  FileManager(const NublobDBOptions& db_options, BlobFileSet* blob_file_set)
...
class TableBuilderTest : public testing::Test {
 public:
  TableBuilderTest()
    ...
    std::map<uint32_t, NublobCFOptions> cfs{{0, cf_options_}};
    db_impl_.reset(new NublobDBImpl(db_options_, tmpdir_));
    ...
    table_factory_.reset(new NublobTableFactory(
  ...
  std::unique_ptr<NublobDBImpl> db_impl_;
...
// Before NublobDBImpl initialized, table factory should return base table
// builder.
