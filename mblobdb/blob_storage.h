...
class BlobStorage {
 public:
  ...
  BlobStorage(const NublobDBOptions& _db_options,
              const NublobCFOptions& _cf_options, uint32_t cf_id,
  ...
  const NublobDBOptions& db_options() { return db_options_; }

  const NublobCFOptions& cf_options() { return cf_options_; }
  ...
  // Must call before NublobDBImpl initialized.
