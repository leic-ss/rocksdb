...
void NublobDBOptions::Dump(Logger* logger) const {
  ROCKS_LOG_HEADER(logger, "NublobDBOptions.dirname                    : %s",
                   dirname.c_str());
  ROCKS_LOG_HEADER(logger, "NublobDBOptions.disable_background_gc      : %d",
                   static_cast<int>(disable_background_gc));
  ROCKS_LOG_HEADER(logger,
                   "NublobDBOptions.max_background_gc          : %" PRIi32,
                   max_background_gc);
  ROCKS_LOG_HEADER(logger,
                   "NublobDBOptions.purge_obsolete_files_period_sec: %" PRIu32,
                   purge_obsolete_files_period_sec);
  ROCKS_LOG_HEADER(logger,
                   "NublobDBOptions.titan_stats_dump_period_sec: %" PRIu32,
                   titan_stats_dump_period_sec);
}

NublobCFOptions::NublobCFOptions(const ColumnFamilyOptions& cf_opts,
...
void NublobCFOptions::Dump(Logger* logger) const {
  ROCKS_LOG_HEADER(logger,
                   "NublobCFOptions.min_blob_size                : %" PRIu64,
                   min_blob_size);
  ...
  ROCKS_LOG_HEADER(logger, "NublobCFOptions.blob_file_compression        : %s",
                   compression_str.c_str());
  ROCKS_LOG_HEADER(logger,
                   "NublobCFOptions.blob_file_target_size        : %" PRIu64,
                   blob_file_target_size);
  ROCKS_LOG_HEADER(logger, "NublobCFOptions.blob_cache                   : %p",
                   blob_cache.get());
  if (blob_cache != nullptr) {
    ROCKS_LOG_HEADER(logger, "%s", blob_cache->GetPrintableOptions().c_str());
  }
  ROCKS_LOG_HEADER(logger,
                   "NublobCFOptions.max_gc_batch_size            : %" PRIu64,
                   max_gc_batch_size);
  ROCKS_LOG_HEADER(logger,
                   "NublobCFOptions.min_gc_batch_size            : %" PRIu64,
                   min_gc_batch_size);
  ROCKS_LOG_HEADER(logger, "NublobCFOptions.blob_file_discardable_ratio  : %lf",
                   blob_file_discardable_ratio);
  ROCKS_LOG_HEADER(logger, "NublobCFOptions.sample_file_size_ratio       : %lf",
                   sample_file_size_ratio);
  ROCKS_LOG_HEADER(logger,
                   "NublobCFOptions.merge_small_file_threshold   : %" PRIu64,
  ...
  ROCKS_LOG_HEADER(logger, "NublobCFOptions.blob_run_mode                : %s",
